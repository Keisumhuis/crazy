...
bool testManualCommitAndAbort()
{
    ...
    {
        MVCCLockWrapper<std::string> str("hello");
...
bool testGetCurrentValue()
{
    TEST_SUITE("获取当前值测试")
    {
        MVCCLockWrapper<int> counter(10);
...
bool testMoveSemantics()
{
    TEST_SUITE("移动语义测试")
    {
        MVCCLockWrapper<std::vector<int>> vec;
...
bool testReadReference()
{
    TEST_SUITE("引用读取测试")
    {
        MVCCLockWrapper<std::string> str("test");
...
bool testReadWriteConcurrency()
{
    TEST_SUITE("读写混合并发测试")
    {
        MVCCLockWrapper<int> counter(42);
...
bool testComplexType()
{
    TEST_SUITE("复杂类型测试")
    {
        ...
        MVCCLockWrapper<Person> person(Person{ "Alice", 25, {"reading"} });
